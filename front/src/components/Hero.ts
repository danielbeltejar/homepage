...
if (i < txt.length) { typeEffectElem.innerHTML +=txt.charAt(i); i++; setTimeout(typeWriter, speed); } }; if
  (window.innerWidth < 1024) { typeWriter(); } else { const typeEffectElem=document.getElementById('type-effect'); if
  (typeEffectElem) { typeEffectElem.innerHTML='Daniel Beltejar' ; setTimeout(typeWriter, 2200); } } }, []); return (
